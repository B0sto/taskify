...
def registeruser(mail,password,username):
    bcrypt=Bcrypt(app)

    user=User(mail=mail,username=username,password=bcrypt.generate_password_hash(password).decode('utf-8'))
    db.session.add(user)
    db.session.commit()
    ...
    if(request.method=='GET'):
        ...
        if('code' in request.form.keys()):
            ...
            if(int(request.form.get('code'))==int(session['sentcode'])):
                ...
                registeruser(request.form.get('mail'), session['registerpassword'] , session['registerusername'])
                session.pop('registerpassword' , None)
                session.pop('registerusername', None)
                session.pop('sentcode' , None)
            ...
            if (result != None):
                ...
                if (credstatus == True):
                    ...
                    session['registerusername']=request.form.get('username')
...
def loggedinpage():
    if(request.method=='POST'):
        ...
        return render_template('loggedin.html', user=User.query.filter_by(mail=session['username']).first().username,
